...
foreach($pieces as $piece){
    if($container == 0){
        ...
        $list_descriptions[] = $piece;
    }
    $container = ($container == 0) ? 1 : 0 ;
}
unset($pieces,$piece,$container);

$items = array();
foreach($list_items as $key => $item){
    $object = (object) null;
    $object->item = $list_items[$key];
    $object->description = $list_descriptions[$key];
    $items[] = $object;
}
unset($list_items,$list_descriptions,$object,$key);
$counter = 0;
$elements = array();
$classes = array();
foreach($items as $key => $item){
    $object = (object) null;
    $flag = 0;
    $parts_1 = substr($item->item,0,strpos($item->item,'('));
    $parts_2 = str_replace($parts_1,'',$item->item);
    
    if(strpos($parts_1,' ') !== 0){
        $returns = trim(substr($parts_1,0,strpos($parts_1,' ')));
        $name = trim(str_replace($returns . ' ','',$parts_1));
    }else{
        $returns = 'unknown';
        $name = trim($parts_1);
    }
    $object->name = $name;
    $object->return_type = $returns;
    ...
    if(strpos($name,'::') !== false){
        ...
        $object->element_type = 'method';
    }else{
        $object->element_type = 'function';
    }
    unset($parts_1,$pieces,$name,$returns,$class,$method);
    
    $optionals = substr_count($parts_2, ']');
    $parts_2 = trim(str_replace(array('(',')','[',']'),'',trim($parts_2)));
    $pieces = array();
    if($parts_2 != ''&& $parts_2 != 'void'){
        $pieces = (strpos($parts_2, ',') === 0 ) ? array($parts_2): explode(',',$parts_2);
        if(trim($pieces[0]) == ''){
            array_shirt($pieces);
        }
    ...
    if(count($pieces) > 0){
        foreach($pieces as $piece){
            $parameters_list[] =(strpos(trim($piece), ' ') !== false ) ?  explode(' ',trim($piece)) : array(trim($piece));
        }
    }
    $requireds = (count($parameters_list) == 0) ? 0 : count($parameters_list) - $optionals;
    unset($parts_2,$pieces,$piece,$optionals);

    $parameters = array();
    foreach($parameters_list as $item_array){
        $loop_object = (object) null;
        switch(count($item_array)){
            case 1:
                $loop_object->name = $item_array[0];
                $loop_object->type = $item_array[0];
                $loop_object->default_value = null;
                $flag = 1;
                break;
            case 2:
                $loop_object->name = $item_array[1];
                $loop_object->type = $item_array[0];
                $loop_object->default_value = null;
                $flag = 1;
                break;
            default:
                if(count($item_array) > 3){
                    $loop_object->name = $item_array[1];
                    $loop_object->type = $item_array[0];
                    $loop_object->default_value = trim(str_replace(array($item_array[0] . ' ' . $item_array[1],$item_array[2],'"','`'),'',implode(' ',$item_array)));
                }elseif(count($item_array) == 3){
                    $loop_object->name = $item_array[1];
                    $loop_object->type = $item_array[0];
                    $loop_object->default_value = trim(str_replace(array($item_array[0] . ' ' . $item_array[1],'=','"'),'',implode(' ',$item_array)));
                    $flag = 1;
                }else{
                    $loop_object->name = 'unknown';
                    $loop_object->type = 'unknown';
                    $loop_object->default_value = 'unknown';
                    $flag = 1;
                }
                break;
        }
        if($requireds > 0){
            $loop_object->required = 'true'; 
            $requireds--;
        }else{
            $loop_object->required = 'false'; 
        }
        $loop_object->allowed_values_service = null;
        $loop_object->value_service = null;
        $loop_object->default_component = null;
        $loop_object->component = null;
    
        $parameters[] = $loop_object;
    }
    
    $object->parameters = $parameters;
    unset($parameters_list,$parameters,$requireds,$optionals,$item_array,$loop_object);
    
    if(strpos($item->description,'Alias of') !== false){
        $alias = trim(str_replace(array('(',')','Alias of'),' ',$item->description));
        $alias = (strpos($alias,' ') !== false) ? explode(' ',$alias) : array($alias);
        $alias = $alias[0];
        $object->alias = $alias;
    }
    ...
    $object->flagged = $flag;
    unset($description,$flag,$requireds,$optionals,$item_array,$loop_object);
...
foreach($classes as $key => $class){
    if(array_key_exists($class.'::__construct',$elements) == true){
        $elements[$class] = $elements[$class.'::__construct'];
        $elements[$class]->name = $class;
        $elements[$class]->element_type = 'class';
    }
}
unset($classes,$key,$class);
foreach($elements as $key => $element){
    $alias='';
    if(isset($element->alias)){
        $alias = $element->alias;
        if(isset($elements[$alias]->alias) && $elements[$alias]->name != $element->name){
            $alias = $elements[$alias]->alias;
            if(isset($elements[$alias]->alias) && $elements[$alias]->name != $element->name){
                $alias = $elements[$alias]->alias;
                if(isset($elements[$alias]->alias) && $elements[$alias]->name != $element->name){
                    $alias = $elements[$alias]->alias;
        ...
        $elements[$element->name]->parameters = $elements[$alias]->parameters;
        $elements[$element->name]->return_type = $elements[$alias]->return_type;
        $elements[$element->name]->description .= $elements[$alias]->description;
    }
}
unset($element,$key,$class,$alias);
foreach($elements as $key => $object){
    if(!isset($object->alias)){continue;}
    $sequode = (object) null;
    ...
    if($object->return_type != 'void' && $object->return_type != ''){
        $member = $object->return_type;
        $sequode->output_object_detail->$member = (object) null;
        $sequode->output_object_detail->$member->return_type = $object->return_type;
        $sequode->output_object_detail->$member->display_name = $object->return_type;
        $sequode->output_object_detail->$member->default_value = null;
    }
    $sql "
UPDATE sequodes_php 
SET 
detail = '".json_encode($sequode->detail)."',
input_object = '".json_encode($sequode->input_object)."',
property_object = '".json_encode($sequode->property_object)."',
output_object = '".json_encode($sequode->output_object)."',
input_object_detail = '".json_encode($sequode->input_object_detail)."',
property_object_detail = '".json_encode($sequode->property_object_detail)."',
output_object_detail = '".json_encode($sequode->output_object_detail)."'
WHERE
name = '".$sequode->name."' LIMIT 1;
