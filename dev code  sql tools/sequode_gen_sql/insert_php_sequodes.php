...
foreach($rows as $key => $value ){
    ...
    if(strpos($value,'::') !== false){
        ...
        if(!array_key_exists($object[0],$classes)){
            $classes[$object[0]] = $insertkey;
            $parent_id = $insertkey;
            if(insertStatement($insertkey,$parent_id,$owner,$object[0],$code_type) != false){
               $insertkey++;
            }
        }else{
            $parent_id = $classes[$object[0]];
        }
    }
    if(insertStatement($insertkey,$parent_id,$owner,$value,$code_type) != false){
       $insertkey++;
    }
}
function insertStatement($id,$parent_id,$owner_id,$name,$code_type){
    static $list;
    if(!is_array($list)){
        $list = array();
    ...
    echo "INSERT INTO `sequode_app`.`sequodes_dev`(`id`,`parent_id`,`owner_id`,`name`,`printable_name`,`coding_type`) VALUES ($id,$parent_id,$owner_id, '$name','$name','$code_type');\n";
    return true;
}