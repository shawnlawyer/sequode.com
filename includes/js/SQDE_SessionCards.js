var SQDE_SessionCards = function(){
	var self = this;
    self.initialized = false;
    self.details_route = 'cards/session/details';
    self.icon = 'session';
    self.initialize = function(){
        if(self.initialized == true){ return;}
        registry.subscribeToUpdates({type:'console', collection:'session_search', call: self.run});
    ...
    self.runCollection = function(collection_key){
        ...
        if(Object.keys(collection).length == 1){
            new SQDE_AjaxCall({route:self.details_route, inputs:[collection[Object.keys(collection)[0]].id]});
        }else{
