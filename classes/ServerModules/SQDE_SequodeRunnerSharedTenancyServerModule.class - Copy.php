<?php
class SQDE_SequodeRunnerSharedTenancyServerModule{
	...
	private static function handleSequodeRoute($request_pieces){
		...
		if(isset($request_pieces[0]) && trim($request_pieces[0]) != ''){
			...
			$sequode_model = SQDE_SequodeOperations::getModel($route, 'name', SQDE_ApplicationProfile::model()->server_account);
			if(!$sequode_model){   
				...
				exit;
			}elseif(SQDE_ApplicationProfile::model()->server_account != SQDE_ApplicationProfile::model()->default_server_account && $sequode_model->safe == 0){
				echo 'Tenancy Error<br />Sequence contains sequodes that can only be run on a "Dedictated Sequode Server". : '.$route;
				exit;
			}else{
				array_shift($request_pieces);
				$parameters = array();
			...
			$sequode_model = SQDE_SequodeOperations::getIndex(SQDE_ApplicationProfile::model()->server_account);
			if(!$sequode_model){
				exit;
			}
		}
